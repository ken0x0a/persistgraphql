...
import { getFragmentNames, getOperationDefinitions, isFragmentDefinition } from './extractFromAST'

import { eliminateInterpolations, findTaggedTemplateLiteralsInJS } from './extractFromJS'
...
export interface ExtractGQLOptions {
  extension?: string
  inJsCode?: boolean
...
export class ExtractGQL {
  ...
  public static isDirectory(pathname: string): Promise<boolean> {
  ...
  public getQueryFragments(
    ...
    const reduceQueryDefinitions = (carry: FragmentDefinitionNode[], definition: DefinitionNode) => {
      ...
      if (isFragmentDefinition(definition) && queryFragmentNames[definitionName.value] === 1) {
        ...
        if (!definitionExists) return [...carry, definition]
      }
...
export const main = (argv: YArgsv) => {
  ...
  if (argv.add_typename) queryTransformers.push(addTypenameTransformer)
  ...
  if (argv.js) options.inJsCode = true

  if (argv.extension) options.extension = argv.extension
